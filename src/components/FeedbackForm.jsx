...
export default function Feedback() {
  ...
  const feedbackSubmit = async () => {
    if (text.description.length == 0 || text.rating.length == 0) {
      ...
      const { data } = await axios
        .post("https://hypekar-backend.herokuapp.com/feedback/", text)
  ...
  return (
    ...
    <Flex align={"center"} justify={"center"} mt={10} minH={"100vh"}>
      <Stack>
        {/* <Box  h={500} >
      ...
      >
        <Box>
          <img
            src="https://media.istockphoto.com/photos/customer-review-satisfaction-feedback-survey-concept-rating-service-picture-id1386241242?b=1&k=20&m=1386241242&s=170667a&w=0&h=eSdg_oWKms6jn9EFncmzb9Y_tPgtDAMCOV3ehbcJWFM="
            alt=""
            height={100}
          />
        </Box>
        <Flex mt={5} align={"center"} justify={"center"}>
          <Stack spacing={4} w={"full"} maxW={"md"}>
            <Heading fontFamily={"Oswald"} fontSize={"2xl"} className="mt-3">
              Give your Feedback
            </Heading>
            <FormControl id="email">
              <FormLabel fontFamily={"Oswald"}>Feedback</FormLabel>
              <Textarea
            ...
            <FormControl>
              <FormLabel fontFamily={"Oswald"}>Rating</FormLabel>
