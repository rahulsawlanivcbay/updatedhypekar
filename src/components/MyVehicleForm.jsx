...
export default function MyVehicleForm() {
  ...
  const submitDetails = async () => {
    if (
      text.brand.length == 0 ||
      text.fuel_Type == 0 ||
      text.mobile_number.length == 0 ||
      text.model_Name.length == 0 ||
      text.year_Of_Model.length == 0
    ) {
      alert("please enter the data");
    } else {
      await axios
        .post("https://hypekar-backend.herokuapp.com/carform/", text)
    ...
    <Stack minH={"100vh"} direction={{ base: "column", md: "row" }}>
      <Flex p={8} flex={1} align={"center"} justify={"center"}>
        <Stack spacing={4} w={"full"} maxW={"md"}>
          <Heading fontSize={"2xl"} fontFamily={"Oswald"}>
            Upload Your Vehicle Details
          </Heading>
          <FormControl id="email">
            <FormLabel fontFamily={"Oswald"}>Select Brand</FormLabel>
          ...
          <FormControl>
            <FormLabel fontFamily={"Oswald"}>Select Model</FormLabel>
            <Select
            ...
            >
              {totalModel.length > 0
                ? totalModel.map((e) => (
                    <option color="green" value={e.model_name}>
                      {e.model_name}
                    </option>
                  ))
                : console.log(null)}
          ...
          <FormControl>
            <FormLabel fontFamily={"Oswald"}>Select Fuel Type</FormLabel>
            <Select
              value={text.fuel_Type}
              id="fuel_Type"
          ...
          <FormControl id="password">
            <FormLabel fontFamily={"Oswald"}>Year Of Model</FormLabel>

            <Select
              value={text.year_Of_Model}
              onChange={handlChange2}
              id="year_Of_Model"
            >
              {years.map((year, i) => (
          ...
          <FormControl id="password">
            <FormLabel fontFamily={"Oswald"}>Vehicle Number</FormLabel>
            <Input
          ...
          <FormControl id="password">
            <FormLabel fontFamily={"Oswald"}>Mobile Number</FormLabel>
            <Input
