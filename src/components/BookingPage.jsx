...
export default function BookingPage() {
  ...
  const handleSameAddress = (isChecked) => {
    if (isChecked) setText({ service_address: text.customer_address });
  };
  ...
  const getBrand = async (e) => {
    const res = await axios.get(
      "https://hypekar-backend.herokuapp.com/carbrand"
    );
  ...
  const getData = async () => {
    const res = await axios.get(
      `https://hypekar-backend.herokuapp.com/carmodel?brand=${model}`
    ); // Here i am passing the model dynamically in the API
  ...
  const service_charge = 1; // Here I declared service charge for 250/-

  const submitDetails = async () => {
    if (
      text.customer_name.length == 0 &&
      text.customer_address.length == 0 &&
      text.service_address.length == 0 &&
      text.time_slot.length == 0 &&
      text.model.length == 0 &&
      text.brand.length == 0
    ) {
      ...
      await axios
        .post("https://hypekar-backend.herokuapp.com/book/", text) // Here I am posting all the data to the API
        .then(toast.success("Booked successfully"))
        .then(
          setText({
            brand: "",
            model_Name: "",
        ...
        ); // here I am making input boxes empty.
    }
  };
  ...
  return (
    <>
      {loading == true ? (
        ...
        <div className="imagecontainer booking_imagecontainer booking_root">
            ...
            <Flex p={8} flex={1} align={"center"} justify={"center"}>
              ...
              >
                ...
                <FormControl>
                  ...
                  <Input
                    value={text.customer_address}
                    onChange={handlChange2}
                    id="customer_address"
                  ></Input>
                </FormControl>
                <Checkbox onChange={(e) => handleSameAddress(e.target.checked)}>
                  Service address same as customer address
                </Checkbox>
                <FormControl>
                  <FormLabel>Service Address</FormLabel>
                  <Input
                    value={text.service_address}
                    onChange={handlChange2}
                    id="service_address"
                  ></Input>
                </FormControl>
                <div className="row mx-0 mt-2">
                  <div className="col-12 col-lg-6 col-md-6 ps-0">
                    <FormControl id="email">
                      ...
                      >
                        {brand.length > 0
                          ? brand.map((e) => <option>{e.brand}</option>)
                          : console.log(null)}
                      </Select>
                    </FormControl>
                  </div>
                  <div className="col-12 col-lg-6 col-md-6 pe-0">
                    <FormControl>
                ...
                <div className="row mx-0 mt-2">
                  ...
                  <div className="col-12 col-lg-6 col-md-6 pe-0">
                    <FormControl>
                      ...
                      <Select
                        value={text.add_services}
                        id="add_service"
                        onChange={handlChange2}
                      >
                        <option value="Fuel">Additional Services</option>
                ...
                <Calendar onChange={changeDate} minDate={new Date()} />
                {newDate.length == 0 ? (
                  console.log(null)
                ) : (
                  <FormControl>
                    <FormLabel>Service Date</FormLabel>
                    <Input value={newDate}></Input>
                  </FormControl>
                )}
                ...
                <FormControl>
                  ...
                  >
                    ...
                    <option value="12:00 PM to 1:00 PM">
                      12:00 PM to 1:00 PM
                    </option>
                    <option value="1:00 PM to 2:00PM">1:00 PM to 2:00PM</option>
                    <option value="2:00 AM to 3:00 PM">
                      2:00 PM to 3:00 PM
                    </option>
                    <option value="3:00 PM to 4:00 PM">
                      3:00 PM to 4:00 PM
                    </option>
